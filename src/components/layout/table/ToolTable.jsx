...
export const ToolTable = ({ departmentID, yearID }) => {
	...
	const handleLoad = async () => {
		try {
			...
			const res = await fetchAPI(
				`/api/practical-tools/departments/${departmentID}/years/${yearID}`
			);
			setValues(res.data);
		} catch (error) {
			toast.error(error);
	...
	const handlePrint = async () => {
		try {
			...
			const res = await fetchPdf(
				`/api/report/tools/departments/${departmentID}/years/${yearID}`
			);
		} catch (error) {
			toast.error(error);
		} finally {
