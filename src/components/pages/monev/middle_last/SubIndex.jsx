...
export const SubMiddleLastMonev = () => {
	const { setIsLoading, isSuccess } = useLoading();
	const { yearID } = useParams();
	const [year, setYear] = useState(null);
	const [values, setValues] = useState(null);
	const links = [
		`/middle-last-monev/teacher-attendences/years/${yearID}`,
		`/middle-last-monev/student-attendences/years/${yearID}`,
		`/middle-last-monev/plans/years/${yearID}`,
	];
	...
	return (
		<>
			<DashboardLayout
				header={`Monev Akhir Sebelum UAS: ${year?.name}`}
			>
				<Card>
					<CardBody>
